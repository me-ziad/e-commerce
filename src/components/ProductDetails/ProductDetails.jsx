...
export default function ProductDetails() {
  ...
  async function relatedProducts() {
    try {
      ...
      let relatedProductt = relatedProduct.filter(
          ...
          <div className="container">
            <div className=" lg:flex rounded-lg shadow-xl border lg:items-center dark:text-gray-300  p-5">
              <div className="  lg:w-1/4">
                <Slider {...settings}>
                  {products.images.map((image, indix) => (
                    <img key={indix} className=" w-full" src={image} alt="" />
                  ))}
                </Slider>
              </div>
              <Helmet>
              ...
              <div className=" lg:w-3/4 lg:p-5  lg:ps-20">
                ...
                <div className=" flex justify-between items-center mt-4">
                  <div className=" flex flex-col">
                    <span>
                      ...
                      {Array.from({ length: Math.round(products.ratingsAverage) }).map((_, index) => (
                        <svg
                          key={index}
                  ...
                  <i
                    onClick={() => postWhishList(products.id)}
                ...
                <button
                  ...
                  className="w-full rounded-md bg-main lg:ps-5 lg:pe-5 pt-1 mt-8 pb-1 hover:bg-green-400 text-white"
                >
                  {t('AddtoCart')}
                </button>
              </div>
        ...
        <>
          ...
          <div className=" flex  flex-wrap gap-y-4 ">
            {relatedProductCategory.map((product) => (
              <div
                key={product.id}
                className="m-auto w-full lg:ms-auto p-2 md:w-1/2  lg:w-1/5"
              >
                <div className=" shadow-md group overflow-hidden group dark:bg-slate-800 dark:text-gray-400  w-full p-3 hover:border-main hover:border-[1px] hover:shadow-lg transition-all hover:border-solid rounded-sm">
                  ...
                  <button
                    ...
                    className="w-full text-center hover:bg-green-400 group-hover:translate-y-0 translate-y-full duration-500 group-hover:opacity-100 pt-2 pb-2 ps-4 pe-4 opacity-0 bg-main text-white mt-6 rounded"
                  >
                    {t('AddtoCart')}
                  </button>
                </div>
