...
export default function Cart() {
  ...
  return (
    <>
      <h1 className='text-gray-500 dark:text-gray-400 text-xl lg:text-xl px-10 font-semibold mb-4'>{t('MyCart')}</h1>
      {cart ? <>
        {cart.data?.products.length === 0 ? <>
          <div className="text-center flex flex-col  text-gray-500 dark:text-gray-400 mt-40">
          ...
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                ...
                {cart.data?.products.map((item, indix) => (
                  <tr key={indix} className="bg-white dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600 border-b dark:text-gray-400 border-gray-200 hover:bg-gray-50">
                    <td className="p-1">
                    ...
                    <td className="px-6 py-1">
                      <button onClick={() => deleteCart(item.product.id)} className="font-medium text-red-600 dark:text-red-500 hover:underline"><i className="fa-solid me-2 fa-trash"></i>{t('Remove')}</button>
                    </td>
                  </tr>
                ))}
            ...
            <div className="w-full ps-10 lg:ps-0 py-6 lg:flex-row flex justify-center lg:justify-around flex-col">
              ...
              <div className="mt-4">
                <h1 className='uppercase font-bold text-xl ps-2 lg:text-2xl text-cyan-800'>{t('totalprice')} <span className='ms-4 text-[#FF9119]'>{cart.data?.totalCartPrice} {t('EGY')}</span ></h1>
              </div>
            ...
            <div className="w-full flex justify-center mt-5">
              ...
              <button onClick={() => clearCart()} className=' text-center hover:bg-red-600 pt-2 pb-2 ps-4 pe-4 bg-red-500 mb-4 text-white mt-6 rounded'>{t('RemoveCart')}</button>
            </div>
          </div>
        )}
      </> :
