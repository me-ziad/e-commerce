            ...
            >
              <div className=" flex justify-end me-5 -translate-y-7">
                <i
                  ...
                  className="text-2xl cursor-pointer dark:text-gray-300  text-black fa-solid fa-xmark"
                ></i>
              </div>
              <div className=" flex justify-center py-6 px-10 items-center border-t-gray-200 dark:border-gray-700 border-t border-b  border-b-gray-200 h-full w-ful">
                <div className=" flex-col w-1/2 text-white flex ">
                  <h3 className=" text-main text-3xl mb-2">
                    {brandsDetails.name}
                  </h3>
              ...
              <div className=" flex w-ful translate-y-3 justify-end me-4">
                <button
                  onClick={() => setShowDetails(false)}
                  className=" bg-gray-500 lg:py-2 px-1 py-1  lg:px-4 rounded-md text-white"
                >
                  {t('close')}
                </button>
              </div>
        ...
        <>
          <div className=" flex  flex-wrap lg:p-6 gap-y-4 justify-center text-center">
            {data?.data?.data.map((prand) => (
              <div
                onClick={() => PrandsDetails(prand._id)}
                key={prand._id}
                className="m-auto hover:cursor-pointer  dark:text-gray-200 p-4 lg:p-2 w-full md:w-1/2  lg:w-1/4"
              >
                <div className=" border dark:border-gray-600 group dark:text-gray-200  w-full dark:bg-gray-900 bg-white  hover:shadow-main hover:border-[1px] overflow-hidden hover:border-main hover:shadow-sm  duration-500 transition-all rounded-sm">
                  ...
                  <div className=" px-3">
                    ...
                    <button
                      onClick={() => PrandsDetails(prand._id)}
                      className=" w-full text-center hover:bg-green-400 group-hover:translate-y-0 translate-y-full duration-500 group-hover:opacity-100 py-2 px-4 opacity-0 bg-main text-white mt-2 mb-2 rounded"
                    >
                     {t('viewdetails')}
                    </button>
                  </div>
